...
//记录区 [结束ip (4byte)] + [地区1] + [地区2]                                                             4字节+不定长 phperz.com 
...
static public function getInstance() {
  static $global_iplocation;
  if (!isset($global_iplocation)) {
    $global_iplocation = new ipLocation();
  }
  return $global_iplocation;
}
...
//*
function ipLocation($datfile = "/usr/local/WholetonTM/htdocs/base/data/CoralWry.dat"){
...
function searchip($ip){
 ...
 $startip=strrev(fread($this->fp,4));         //读取当前索引区内的开始ip地址,并将其little-endian的字节序转换成big-endian的字节序 php程序员之家 
 if ($ip < $startip) {
    $lastip=$i - 1;
 }
 else {
